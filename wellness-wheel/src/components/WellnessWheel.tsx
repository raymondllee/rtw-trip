...
const rings: Ring[] = [
  ...
  useEffect(() => {
    const loadOrGenerateSummaries = async () => {
      ...
      const existingUserSummary = summaryStorageService.getUserSummaries(userId);
      
      if (existingUserSummary && existingUserSummary.summaries && Object.keys(existingUserSummary.summaries).length > 0) {
        // Use existing summaries
        setSummaries(existingUserSummary.summaries);
        return;
      }
      ...
      try {
        ...
        // Save summaries to storage for this user
        const userSummary = {
          userId: userData.userId,
          userName: userData.userName,
          summaries: newSummaries,
          lastUpdated: new Date().toISOString(),
          isEdited: false // New summaries are not edited
        };
        summaryStorageService.saveUserSummaries(userSummary);
