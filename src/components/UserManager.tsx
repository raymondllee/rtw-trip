...
const UserManager: React.FC<UserManagerProps> = ({ 
  ...
  const loadUsers = () => {
    const allUsers = summaryStorageService.getAllUsers();
    setUsers(allUsers);
    
    // Load current user data
    if (currentUserId) {
      const userData = summaryStorageService.getUserById(currentUserId);
  ...
  const handleAddUser = () => {
    if (newUserName.trim()) {
      const userId = summaryStorageService.generateUserId(newUserName.trim());
  ...
  const handleSaveEdit = () => {
    if (editingUser && editUserName.trim()) {
      ...
      if (user) {
        ...
        summaryStorageService.saveUser(updatedUser);
        setEditingUser(null);
        setEditUserName('');
        loadUsers();
      }
  ...
  const handleDeleteUser = (userId: string) => {
    if (window.confirm('Are you sure you want to delete this user and all their data?')) {
      summaryStorageService.deleteUser(userId);
      loadUsers();
  ...
  const handleExportUser = (userId: string) => {
    const exportData = summaryStorageService.exportUserData(userId);
  ...
  const handleImportUser = () => {
    if (summaryStorageService.importUserData(importData)) {
      setImportData('');
      setShowImport(false);
      loadUsers();
  ...
  return (
    <div className="relative">
      ...
      {/* Full User Management Panel (Hidden by default, can be shown with a button) */}
              ...
              <div className="flex items-center gap-3">
                {editingUser === user.userId ? (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {summaryStorageService.hasSummaries(user.userId) && (
                      <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full">
                        Has Summaries
                      </span>
                    )}
                    {summaryStorageService.isSummaryEdited(user.userId) && (
