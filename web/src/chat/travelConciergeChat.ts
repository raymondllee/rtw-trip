...
import { collection, addDoc, doc, setDoc, getDoc, getDocs, query, where, orderBy, limit, updateDoc, deleteDoc, Timestamp } from 'firebase/firestore';

import { getRuntimeConfig } from '../config';
import { db } from '../firebase/config';
import { StatePersistence } from '../statePersistence';

const { endpoints, apiBaseUrl } = getRuntimeConfig();
const CHAT_API_URL = endpoints.chat;
const CHANGES_API_URL = endpoints.itineraryChanges;
const TITLE_API_URL = `${apiBaseUrl}/api/generate-title`;

export class TravelConciergeChat {
  ...
  async handleSubmit(e, isFloating = false, isColumn = false) {
    ...
    try {
      ...
      if (error.message.includes('timeout')) {
        ...
        errorMessage += `Please make sure the backend server is running on ${apiBaseUrl}`;
      } else {
