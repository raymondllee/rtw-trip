...
IMPORTANT: Return ONLY the JSON array, no markdown formatting, no explanation text, no code fences. Just the raw JSON array starting with [ and ending with ].`;
    ...
    return `
      <div class="traveler-section">
        ...
        <div class="traveler-inputs">
          ...
          <button class="btn-sm btn-secondary" id="save-travelers-btn">💾 Save</button>
        </div>
        <div class="traveler-note">
          <small>ℹ️ Costs generated by AI will use this traveler count. Regenerate existing costs to update them.</small>
        </div>
      </div>
    `;
  }
  ...
  private attachEventListeners() {
    ...
    // Save travelers button
    const saveTravelersBtn = this.container.querySelector('#save-travelers-btn');
    saveTravelersBtn?.addEventListener('click', async () => {
      ...
      // Visual feedback
      const btn = saveTravelersBtn as HTMLButtonElement;
      const originalText = btn.innerHTML;
      btn.innerHTML = '💾 Saving...';
      btn.disabled = true;

      try {
        // Save to Firestore via callback (if available)
        if (this.onTripDataUpdate) {
          await this.onTripDataUpdate(this.tripData);
          btn.innerHTML = '✓ Saved';
          btn.style.background = '#28a745';
        } else {
          // No callback available, just show success
        ...
        alert('Failed to save traveler data: ' + (error instanceof Error ? error.message : 'Unknown error'));
      } finally {
